...
function exp_poincare_distance(u, i, norm_diff, vec_theta_norm, vec_beta_norm)
  return exp( acosh( 1 + 2 * ( norm_diff / (vec_theta_norm[u] * vec_beta_norm[i]) )^2 ) )
end

function exp_minus_poincare_distance(u, i, norm_diff, vec_theta_norm, vec_beta_norm)
  return exp( -acosh( 1 + 2 * ( norm_diff / (vec_theta_norm[u] * vec_beta_norm[i]) )^2 ) )
end

function cosine_similarity(u_id, i_id, matTheta, matBeta)
  return dot(matTheta[u_id,:], matBeta[i_id,:])
end

function exp_minus_cosine_similarity(u_id, i_id, matTheta, matBeta)
  ret = 0
  a = -dot(matTheta[u_id,:], matBeta[i_id,:])
  if a < -10e4
    ret = 0
  else
    ret = exp(a)
...
function update_matTheta_default(matTheta::Array{Float64,2}, matBeta::Array{Float64,2},
  ...
  for itr=1:length(sampled_i)
    ...
    dot_prod = dot(matTheta[u_id,:], matBeta[i_id,:])
    if isnan(dot_prod)
      println("Theta dot_prod is nan")
    elseif dot_prod > 40
      vec_partial_L_by_d = -dot(matTheta[u_id,:], matBeta[i_id,:]) - (1 + alpha*sampled_v[itr]) * dot_prod
    elseif dot_prod < -40
      vec_partial_L_by_d = -dot(matTheta[u_id,:], matBeta[i_id,:])
    else
      vec_partial_L_by_d = -dot(matTheta[u_id,:], matBeta[i_id,:]) - (1 + alpha*sampled_v[itr]) * log(1+exp_minus_cosine_similarity(u_id, i_id, matTheta, matBeta))
    end

    if isnan(vec_partial_L_by_d)
      println("matTheta")
      println(matTheta[u_id,:])
      println(matBeta[i_id,:])
      println(sampled_v[itr])
      println(dot(matTheta[u_id,:], matBeta[i_id,:]))
      println(exp_minus_cosine_similarity(u_id, i_id, matTheta, matBeta))
    end

    # Compute vec_partial_d_by_theta
    vec_partial_d_by_theta_u = matBeta[i_id, :]
  ...
  for u = 1:usr_idx_len
    if sum(isnan(mat_partial_L_by_theta[u,:]))>0
      println("matTheta mat_partial_L_by_theta")
      println(mat_partial_L_by_theta[u,:])
    end
    if sum(isnan(matTheta[usr_idx[u],:]))>0
      println("matTheta matTheta")
      println(matTheta[usr_idx[u],:])
    end
    matTheta[usr_idx[u],:] += lr * (mat_partial_L_by_theta[u,:] - 0.1 * matTheta[usr_idx[u],:])
  end
  #matTheta[usr_idx,:] -= lr * 0.1 * matTheta[usr_idx,:]
...
function update_matBeta_default(matTheta::Array{Float64,2}, matBeta::Array{Float64,2},
  ...
  for itr=1:length(sampled_i)
    ...
    # Compute vec_partial_d_by_theta
    vec_partial_d_by_beta_u = matTheta[u_id, :]
  ...
  for i = 1:itm_idx_len
    if sum(isnan(mat_partial_L_by_beta[i,:]))>0
      println("matBeta mat_partial_L_by_beta")
      println(mat_partial_L_by_beta[i,:])
    end
    if sum(isnan(matBeta[itm_idx[i],:]))>0
      println("matBeta matBeta")
      println(matBeta[itm_idx[i],:])
    end
    matBeta[itm_idx[i],:] += lr * (mat_partial_L_by_beta[i,:] - 0.1 * matBeta[itm_idx[i],:])

  end
  #matBeta[itm_idx,:] -= lr * 0.1 * matBeta[itm_idx,:]
...
function update_matTheta(matTheta::Array{Float64,2}, matBeta::Array{Float64,2},
  ...
  for itr=1:length(sampled_i)
    ...
    f_ui = exp_poincare_distance(u, i, norm_diff, vec_theta_norm, vec_beta_norm)
...
function update_matTheta2(matTheta::Array{Float64,2}, matBeta::Array{Float64,2},
  ...
  for itr=1:length(sampled_i)
    ...
    # Compute vec_partial_L_by_d
    vec_sampled_neg_i_id = zeros(Int64, 11)
    vec_f_ui = zeros(Float64, 11)
    neg_i_itr = 1
    while neg_i_itr < 11
      sampled_neg_i = rand(collect(1:itm_idx_len))
      sampled_neg_i_id = itm_idx[sampled_neg_i]
      if matX_train[u_id, sampled_neg_i_id] < matX_train[u_id, i_id]
        vec_sampled_neg_i_id[neg_i_itr] = sampled_neg_i
        neg_i_itr += 1
        #vec_f_ui[neg_i_itr] = exp( -acosh( 1 + 2 * ( norm(matTheta[u_id,:] - matBeta[i_id,:]) / (vec_theta_norm[u] * vec_beta_norm[sampled_neg_i]) )^2 ) )
        vec_f_ui[neg_i_itr] = exp_minus_poincare_distance(u, sampled_neg_i, norm(matTheta[u_id,:] - matBeta[i_id,:]), vec_theta_norm, vec_beta_norm)
      end
    end
    vec_sampled_neg_i_id[neg_i_itr] = i
    #vec_f_ui[neg_i_itr] = exp( -acosh( 1 + 2 * ( norm_diff / (vec_theta_norm[u] * vec_beta_norm[i]) )^2 ) )
    vec_f_ui[neg_i_itr] = exp_minus_poincare_distance(u, i, norm_diff, vec_theta_norm, vec_beta_norm)
    vec_partial_L_by_d = -1 + vec_f_ui[neg_i_itr] / sum(vec_f_ui)
...
function PoincareMF(model_type::String, K::Int64, M::Int64, N::Int64,
  ...
  while IsConverge == false && itr < MaxItr
    ...
    #
    if sum(matTheta .> 10e2) + sum(matTheta .< -10e2) > 0
      println("1 matTheta is nan !!!")
    end
    if sum(matBeta .> 10e2) + sum(matBeta .< -10e2) > 0
      println("1 matBeta is nan !!!")
    end

    update_matTheta_default(matTheta, matBeta, vecGamma, vecDelta, sampled_i, sampled_j, sampled_v, alpha, K, usr_idx_len, itm_idx_len, usr_idx, itm_idx)
    update_matBeta_default(matTheta, matBeta, vecGamma, vecDelta, sampled_i, sampled_j, sampled_v, alpha, K, usr_idx_len, itm_idx_len, usr_idx, itm_idx)

    if sum(matTheta .> 10e2) + sum(matTheta .< -10e2) > 0
      println("2 matTheta is nan !!!")
    end
    if sum(matBeta .> 10e2) + sum(matBeta .< -10e2) > 0
      println("2 matBeta is nan !!!")
    end
    ...
    if mod(itr, check_step) == 0 && check_step > 0
      ...
      #valid_precision[indx,:], valid_recall[indx,:] = evaluatePRPF(matX_valid, matX_train, matTheta, matBeta, vecGamma, vecDelta, topK, alpha)
      valid_precision[indx,:], test_recall[indx,:] = evaluatePRPF(matX_valid, matX_train, matTheta, matBeta, topK, 10., 1.)
      ...
      if findmax(valid_precision[:,1])[2] == Int(itr / check_step)
        ...
        test_precision[indx,:], test_recall[indx,:] = evaluatePRPF(matX_test, matX_train, matTheta, matBeta, topK, 10., 1.)
