...
function LogisticMF(model_type::String, K::Int64, M::Int64, N::Int64,
  ...
  while IsConverge == false && itr < MaxItr
    ...
    matBeta[itm_idx,:] += lr * partial_beta# ./ sqrt(grad_sqr_sum_beta(itm_idx,:]));
    grad_sqr_sum_beta += partial_beta .^ 2

    # Update vecBiasU
    partial_biasI = sum(matTmp, 1)[:]
    vecBiasI[itm_idx] += lr * partial_biasI# ./ sqrt(grad_sqr_sum_biasI(itm_idx)));
    grad_sqr_sum_biasI += partial_biasI .^ 2
    ...
    matTheta[usr_idx,:] += lr * partial_theta# ./ sqrt(grad_sqr_sum_theta(usr_idx,:]));
    grad_sqr_sum_theta += partial_theta .^ 2

    # Update vecBiasU
    partial_biasU = sum(matTmp, 2)[:]
    vecBiasU[usr_idx] += lr * partial_biasU# ./ sqrt(grad_sqr_sum_biasU(usr_idx)));
    ...
    if mod(itr, check_step) == 0 && check_step > 0
      ...
      #valid_precision[indx,:], valid_recall[indx,:] = evaluatePRPF(matX_valid, matX_train, matTheta, matBeta, vecBiasU, vecBiasI, topK, alpha)
      ...
      if findmax(valid_precision[:,1])[2] == Int(itr / check_step)
        ...
        println("Testing ... ")
        indx = Int(itr / check_step)
        #test_precision[indx,:], test_recall[indx,:] = evaluatePoincareMF(matX_test, matX_train, matTheta, matBeta, vecBiasU, vecBiasI, topK, alpha)
